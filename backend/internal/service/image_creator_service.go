...
type ImageCreatorAPIKeyLookup interface {
	...
	// ResolveImageGenerationAPIKey returns a copy of apiKey whose Group is a
	// valid OpenAI image-generation-enabled group, or an error if none qualifies.
	ResolveImageGenerationAPIKey(ctx context.Context, apiKey *APIKey) (*APIKey, error)
}
...
func NewImageCreatorService(repo ImageCreatorRepository, apiKeyService *APIKeyService, cfg *config.Config, storeFactories ...BackupObjectStoreFactory) *ImageCreatorService {
...
func NewImageCreatorServiceLegacy(repo ImageCreatorRepository, apiKeyService *APIKeyService, cfg *config.Config) *ImageCreatorService {
	return NewImageCreatorService(repo, apiKeyService, cfg)
}
...
func (s *ImageCreatorService) CreateTask(ctx context.Context, userID int64, input ImageCreatorCreateTaskInput) (*ImageCreatorTask, error) {
	...
	apiKey, err = s.validateImageCreatorAPIKeyForUser(ctx, apiKey, userID)
	if err != nil {
		return nil, err
	}
...
func (s *ImageCreatorService) validateImageCreatorAPIKeyForUser(ctx context.Context, apiKey *APIKey, userID int64) (*APIKey, error) {
	if apiKey == nil {
		return nil, infraerrors.NotFound("API_KEY_NOT_FOUND", "api key not found")
	}
	...
	resolved, err := s.apiKeys.ResolveImageGenerationAPIKey(ctx, apiKey)
	if err != nil {
		return nil, err
	}
...
func (s *ImageCreatorService) processClaimedTask(ctx context.Context, task *ImageCreatorTask) error {
	...
	apiKey, err = s.validateImageCreatorAPIKeyForUser(ctx, apiKey, task.UserID)
	if err != nil {
